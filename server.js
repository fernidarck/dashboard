...
app.get('/api/webchat/widget.js', (req, res) => {
  res.setHeader('Content-Type', 'application/javascript; charset=utf-8');
  res.setHeader('Cache-Control', 'public, max-age=3600');
  
  const host = req.get('host') || 'ycloud-dashboard.83aqlq.easypanel.host';
...
(function() {
  ...
  const primaryColor = currentScript?.getAttribute('data-color') || '#FF6B00';
  const botName = currentScript?.getAttribute('data-bot-name') || 'Asistente OneControl';
  const waPhone = currentScript?.getAttribute('data-whatsapp') || '35154362';
  const welcomeMsg = currentScript?.getAttribute('data-welcome') || '¡Hola! 👋 Bienvenido a OneControl. ¿En qué te podemos asesorar hoy? Tenemos mesitas de noche modernas, cajones ocultos NFC y más.';
  const apiUrl = '${baseUrl}/api/webchat/message';

  let sessionId = localStorage.getItem('oc_chat_session') || ('web_' + Date.now() + '_' + Math.random().toString(36).substr(2, 6));
  ...
  style.innerHTML = \`
    ...
    .oc-chat-input { flex: 1; border: 1px solid #cbd5e1; border-radius: 20px; padding: 10px 16px; font-size: 13px; outline: none; transition: border-color 0.2s; }
    .oc-chat-input:focus { border-color: \${primaryColor}; }
  ...
  const btn = document.getElementById('ocChatBtn');
  const box = document.getElementById('ocChatBox');
  const close = document.getElementById('ocChatClose');
  const msgs = document.getElementById('ocChatMsgs');
  const input = document.getElementById('ocChatInput');
  const send = document.getElementById('ocChatSend');
